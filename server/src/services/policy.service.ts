...
export class PolicyService {
  ...
  static async existsByNumber(policyNumber: string) {
    const doc = await PolicyModel.exists({
      policyNumber
    });
    return !!doc;
  }
  ...
  static async list(query: ListPoliciesQuery) {
    ...
    const [items, total] = await Promise.all([PolicyModel.find(filter).sort(sort).skip(skip).limit(limitNum), PolicyModel.countDocuments(filter)]);
    return {
      items,
      total,
      page: pageNum,
  ...
  static async update(id: string, input: UpdatePolicyInput): Promise<IPolicy | null> {
    ...
    const update: UpdateQuery<IPolicy> = {
      $set: input
    };
    return PolicyModel.findByIdAndUpdate(id, update, {
      new: true
    });
  }
