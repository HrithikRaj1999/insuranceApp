...
const openai = paidEnabled ? new OpenAI({ apiKey }) : null;
...
/**
 * Summarize a claim description in exactly 2 concise sentences.
 * Falls back to a mock summary if OpenAI is disabled or fails.
 */
export async function generateSummary(description: string): Promise<string> {
  ...
  try {
    ...
    warn('error', 'OpenAI call failed—falling back to mock', { error: e?.message || String(e) });
    return mockSummary(description, 'API error');
  }
}

/**
 * Categorize a claim into: auto, property, health, life, liability, other.
 * Falls back to local heuristic categorization if OpenAI is disabled or fails.
 */
export async function categorizeClaimType(description: string): Promise<string> {
  ...
  try {
    ...
    warn('error-cat', 'OpenAI categorize failed—using mock', { error: e?.message || String(e) });
    return mockCategorize(description);
  }
}
 ...
 * Falls back to local heuristic if OpenAI is disabled or fails.
 */
export async function assessClaimUrgency(
  ...
  try {
    ...
    warn('error-urgency', 'OpenAI urgency failed—using mock', { error: e?.message || String(e) });
    return mockUrgency(description);
  }
}

/** Handy status endpoint/debugger */
