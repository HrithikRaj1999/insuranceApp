...
const errMsg = (e: unknown, fallback = "Internal server error") => e instanceof Error ? e.message : fallback;
const useSigned = process.env.USE_SIGNED_URLS === "true";
export async function getAllClaims(req: Request, res: Response) {
  try {
    const claims = await listClaims();
    const claimsWithUrls = await Promise.all(claims.map(async claim => {
      ...
      if (useSigned) {
        ...
        if (Array.isArray(obj.fileS3Keys) && obj.fileS3Keys.length) {
          obj.fileUrls = await Promise.all(obj.fileS3Keys.map((k: string) => getSignedUrl(k)));
        }
      }
      return obj;
    }));
